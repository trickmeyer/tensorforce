...
def linear(x, size, weights=None, bias=True, l2_regularization=0.0, l1_regularization=0.0, scope='linear',
    ...
    with tf.variable_scope(scope):
        ...
        if l1_regularization > 0.0:
            ...
            tf.losses.add_loss(tf.multiply(weight_l1_t, tf.reduce_sum(tf.abs(weights)), name='l1_loss_weights'))
        else:
            weight_l1_t = None
        ...
        if bias_init is not None:
            ...
            if l1_regularization > 0.0:
                tf.losses.add_loss(tf.multiply(weight_l1_t, tf.reduce_sum(tf.abs(bias)), name='l1_loss_bias'))
